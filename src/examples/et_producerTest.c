...
printf("arg w = %d\n", localEndian);
            ...
            case 6:
                i_tmp = atoi(optarg);
                if (i_tmp != 4 && i_tmp != 6) {
                    printf("Invalid argument to -ver. Evio version must be 4 or 6, defaulting to 4\n");
                }
                else {
                    version = i_tmp;
                }
    ...
    if (optind < argc || errflg || strlen(et_name) < 1) {
        ...
        fprintf(stderr, "          -ver   evio version of data being sent, 4 or 6. 4 is default\n\n");
    ...
    if (version == 4) {
        ...
        if (localEndian) {
            ...
            fakeData[9] = 0xff; // invalid data type
            fakeData[10] = 7;
            fakeData[11] = 8;

            // real evio event format
            fakeData[8] = 4;
            fakeData[9] = 2 << 16 | 1 << 8 | 3; // tag = 2, type = 1 (32 bit unsigned int), num = 3
            fakeData[10] = 7;
            fakeData[11] = 8;
        }
        // big endian
        else {
            fakeData[0] = ET_SWAP32(12);
            fakeData[1] = ET_SWAP32(1);
            fakeData[2] = ET_SWAP32(8);
            fakeData[3] = ET_SWAP32(1);
            fakeData[4] = ET_SWAP32(0);
            fakeData[5] = ET_SWAP32(0x1204);
            fakeData[6] = ET_SWAP32(0);
            fakeData[7] = ET_SWAP32(0xc0da0100);

            // bad format
            fakeData[8] = ET_SWAP32(300);
            fakeData[9] = ET_SWAP32(0xff);
            fakeData[10] = ET_SWAP32(7);
            fakeData[11] = ET_SWAP32(8);

            // real evio event format
            fakeData[8] = ET_SWAP32(4);
            fakeData[9] = ET_SWAP32(2 << 16 | 1 << 8 | 3);
            fakeData[10] = ET_SWAP32(7);
            fakeData[11] = ET_SWAP32(8);
        }
    }
        ...
        // EVIO VERSION 6
        printf("Using evio version 6\n");

        // little endian
        if (localEndian) {
            printf("Using local (little) endian\n");
            // Fake user event in evio block
            fakeData[0] = 18;
            fakeData[1] = 1;
            fakeData[2] = 14;
            fakeData[3] = 1;
            fakeData[4] = 0;
            fakeData[5] = 0x2406; // user event, last block bit, version = 6
            fakeData[6] = 0;
            fakeData[7] = 0xc0da0100;
            fakeData[8] = 32; // record without header in bytes
            fakeData[9] = 0;  // no compression
            fakeData[10] = 0;  // user register 1
            fakeData[11] = 0;
            fakeData[12] = 0;  // user register 2
            fakeData[13] = 0;

            // bad format
            fakeData[14] = 300;  // length is too big
            fakeData[15] = 0xff; // invalid data type
            fakeData[16] = 7;
            fakeData[17] = 8;

            // real evio event format
            fakeData[14] = 4;
            fakeData[15] = 2 << 16 | 1 << 8 | 3; // tag = 2, type = 1 (32 bit unsigned int), num = 3
            fakeData[16] = 7;
            fakeData[17] = 8;
        }
        // big endian
        else {
            printf("Using non-local (big) endian\n");
            // Fake user event in evio block
            fakeData[0] = ET_SWAP32(18);
            fakeData[1] = ET_SWAP32(1);
            fakeData[2] = ET_SWAP32(14);
            fakeData[3] = ET_SWAP32(1);
            fakeData[4] = ET_SWAP32(0);
            fakeData[5] = ET_SWAP32(0x2406);
            fakeData[6] = ET_SWAP32(0);
            fakeData[7] = ET_SWAP32(0xc0da0100);
            fakeData[8] = ET_SWAP32(32);
            fakeData[9] = ET_SWAP32(0);
            fakeData[10] = ET_SWAP32(0);
            fakeData[11] = ET_SWAP32(0);
            fakeData[12] = ET_SWAP32(0);
            fakeData[13] = ET_SWAP32(0);

            // bad format
            fakeData[14] = ET_SWAP32(300);
            fakeData[15] = ET_SWAP32(0xff);
            fakeData[16] = ET_SWAP32(7);
            fakeData[71] = ET_SWAP32(8);

            // real evio event format
            fakeData[14] = ET_SWAP32(4);
            fakeData[15] = ET_SWAP32(2 << 16 | 1 << 8 | 3);
            fakeData[16] = ET_SWAP32(7);
    ...
    while (1) {
        ...
        /* write data, set control values here */
            ...
            for (i=0; i < numRead; i++) {
                ...
                /* Send all data even though we only wrote 12 ints. */
                et_event_setlength(pe[i], size);
                memcpy((void *)pdata, (const void *) fakeData, size);
            }
        }
        else {
            for (i = 0; i < numRead; i++) {
